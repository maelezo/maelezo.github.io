...
\section{Purpose of Course}\label{purpose-of-course}
...
special packages called \texttt{dplyr} and \texttt{tidyr} just for data
wrangling. For the sake of simplicity, we will attempt to primarily use
these packages for data wrangling in this course.
...
we will load the \texttt{lubridate} package (another package written by
Hadley Wickam) and use it to convert these two columns to date format
and calculate the difference between them (i.e.~the number of days
between the incident that the death of the Service Member).
...
use visualizations from the ggplot2 package. If you haven't installed
this package yet, run the command
...
package, which has some more user friendly functions.)
